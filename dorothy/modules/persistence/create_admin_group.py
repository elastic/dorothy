...
# Assign an admin role to an Okta group
...
MODULE_DESCRIPTION = "Assign an admin role to an Okta group"
TACTICS = ["Persistence"]

MODULE_OPTIONS = {"id": {"value": None, "required": True, "help": "The unique ID for the Okta group"}}
MODULE = Module(MODULE_OPTIONS)


@persistence.subshell(name="create-admin-group")
@click.pass_context
def create_admin_group(ctx):
    """Assign an Okta administrator role to a group.

    Only the SUPER_ADMIN role can view, assign, or remove admin roles."""


@create_admin_group.command()
def info():
...
def execute(ctx):
    ...
    group_id = MODULE_OPTIONS["id"]["value"]
    ...
    while True:
        choice = click.prompt("[*] Which admin role do you want to assign to the group?", type=int)

        if (choice > 0) and (choice <= len(admin_roles)):
            role_type = admin_roles[choice - 1]

            msg = f"Attempting to assign admin role {role_type} to group {group_id}"
            LOGGER.info(msg)
